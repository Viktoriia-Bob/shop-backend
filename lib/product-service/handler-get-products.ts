...
export async function main() {
  try {
    const productsData = await client.send(new ScanCommand({ TableName: PRODUCTS_TABLE }));
    const stockData = await client.send(new ScanCommand({ TableName: STOCK_TABLE }));
    ...
    const stockMap = new Map(
      (stockData.Items || []).map(item => [item.product_id.S!, Number(item.count.N!)])
    );
